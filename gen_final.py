#!/usr/bin/env python3
"""Generate per-field CDF data for StbCdfContext from dav1d cdf.c."""
import re, sys
...
fields = [
    ('uv_mode', [2,13,15], False), ('partition', [5,4,16], False),
    ('cfl_alpha', [6,16], False), ('txtp_inter1', [2,16], False),
    ('txtp_inter2', [16], False), ('txtp_intra1', [2,13,8], False),
    ('txtp_intra2', [3,13,8], False), ('cfl_sign', [8], False),
    ('angle_delta', [8,8], False), ('filter_intra', [8], False),
    ('seg_id', [3,8], False), ('pal_sz', [2,7,8], False),
    ('color_map', [2,7,5,8], False), ('txsz', [3,3,4], False),
    ('delta_q', [4], False), ('delta_lf', [5,4], False),
    ('restore_switchable', [4], False), ('restore_wiener', [2], False),
    ('restore_sgrproj', [2], False), ('txtp_inter3', [4,2], False),
    ('use_filter_intra', [22,2], False), ('txpart', [7,3,2], False),
    ('skip', [3,2], False), ('pal_y', [7,3,2], False),
    ('pal_uv', [2,2], False), ('intrabc', [2], False),
    ('y_mode', [4,16], False), ('wedge_idx', [9,16], False),
    ('comp_inter_mode', [8,8], False), ('filter', [2,8,8], False),
    ('interintra_mode', [4,4], False), ('motion_mode', [22,4], False),
    ('skip_mode', [3,2], False), ('newmv_mode', [6,2], False),
    ('globalmv_mode', [2,2], False), ('refmv_mode', [6,2], False),
    ('drl_bit', [3,2], False), ('intra', [4,2], False),
    ('comp', [5,2], False), ('comp_dir', [5,2], False),
    ('jnt_comp', [6,2], False), ('mask_comp', [6,2], False),
    ('wedge_comp', [9,2], False), ('ref', [6,3,2], False),
    ('comp_fwd_ref', [3,3,2], False), ('comp_bwd_ref', [2,3,2], False),
    ('comp_uni_ref', [3,3,2], False), ('seg_pred', [3,2], False),
    ('interintra', [7,2], False), ('interintra_wedge', [7,2], False),
    ('obmc', [22,2], False),
    ('mv_classes', [16], True), ('mv_sign', [2], True),
    ('mv_class0', [2], True), ('mv_class0_fp', [2,4], True),
    ('mv_class0_hp', [2], True), ('mv_classN', [10,2], True),
    ('mv_classN_fp', [4], True), ('mv_classN_hp', [2], True),
    ('mv_joint', [4], True), ('kfym', [5,5,16], False),
]

indexed_fields = {'use_filter_intra', 'motion_mode', 'obmc'}

print("/* Auto-generated per-field CDF data from dav1d cdf.c */")
print("/* Generated by gen_final.py - DO NOT EDIT */")
print()
...
for field_name, our_dims, is_mv in fields:
    ...
    if field_name in indexed_fields:
        ...
        for i in range(2):
            for j in range(13):
                ri = i*13+j
                if ri < len(leaf_rows):
                    flat.extend(leaf_rows[ri][:our_inner])
                else:
                    flat.extend([0]*our_inner)
        assert len(flat) == our_total
    elif field_name == 'txsz':
        leaf_rows = extract_leaf_arrays(block)
        leaf_rows = leaf_rows[3:]  # Skip TX_4X4 level (dav1d [4], ours [3])
        flat = []
    ...
    if len(our_dims) == 1:
        ...
        for i in range(our_dims[0]):
            print("    {" + ",".join(f"{v:5d}" for v in flat[i*our_dims[1]:(i+1)*our_dims[1]]) + "},")
        print("};")
    elif len(our_dims) == 3:
        print(f"static const unsigned short stb_av1_{field_name}[{our_dims[0]}][{our_dims[1]}][{our_dims[2]}] = {{")
        s2 = our_dims[2]; s1 = our_dims[1]*s2
        for i in range(our_dims[0]):
            print("    {")
            for j in range(our_dims[1]):
                print("        {" + ",".join(f"{v:5d}" for v in flat[i*s1+j*s2:i*s1+(j+1)*s2]) + "},")
            print("    },")
        print("};")
    elif len(our_dims) == 4:
        print(f"static const unsigned short stb_av1_{field_name}[{our_dims[0]}][{our_dims[1]}][{our_dims[2]}][{our_dims[3]}] = {{")
        s3=our_dims[3]; s2=our_dims[2]*s3; s1=our_dims[1]*s2
        for i in range(our_dims[0]):
            print("    {")
            for j in range(our_dims[1]):
                print("        {")
                for k in range(our_dims[2]):
                    print("            {" + ",".join(f"{v:5d}" for v in flat[i*s1+j*s2+k*s3:i*s1+j*s2+(k+1)*s3]) + "},")
                print("        },")
            print("    },")
        print("};")
    print()

print(f"/* Summary: {ok_count} OK, {warn_count} warnings */", file=sys.stderr)
